class BiomeGenerator {
    ...
    generate() {
        ...
        for(let y = 0; y < this.height; y++) {
            for(let x = 0; x < this.width; x++) {
                const height = this.getNormalizedNoise(heightNoise, x, y, 100);
                const temperature = this.getNormalizedNoise(tempNoise, x, y, 150);
                const humidity = this.getNormalizedNoise(humidityNoise, x, y, 120);
    ...
    getNormalizedNoise(noise, x, y, scale) {
        return (noise.noise2D(x / scale, y / scale) + 1) / 2;
    }
